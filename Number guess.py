...
print(number)
name =input("May I ask you for your name?  ")#asks for the name
def intro():
...
def pick():
    guessesTaken = 0
    while guessesTaken < 6: #if the number is less than 6
        time.sleep(.25)
        ...
        try: #check if a number was entered
            ...
            if guess<=200 and guess>=1: #if they are in range
                guessesTaken=guessesTaken+1 #adds 1 guess each time the player is wrong
                if guessesTaken<6:
                    if guess<number:
                    ...
                    if guess !=  number:
                        ...
                        print("Try again!")
                if guess == number:
                    break #if guess is right, then we're going to jump out of the while block
